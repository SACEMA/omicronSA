...
#    geom_line(aes(y=tot-var, color="del")) +
    ...
    scale_y_continuous("Incidence", trans="log2", breaks = function(ls) 2^seq(1,log(ls[2],2),by=3), minor_breaks = function(ls) 2^seq(0,log(ls[2],2),1)) +
    scale_color_manual(
        NULL, labels = c(del="Est. background", obs="Reported"), values = c(del="firebrick", obs="black")
    ) +
    theme(
