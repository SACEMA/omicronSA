...
ensemble.dt <- yusamp[, .SD, .SDcols = grep("(u|y)_", names(yusamp))][, epi_sample := 1:.N ]

AR.dt <- readRDS(.args[3])
timing <- readRDS(.args[4])
mob.dt <- readRDS(.args[5])

contact_schedule <- mob.dt[timing, on=.(region=province)][between(date,start,start+6)][, .(
  home = 1, work = prod(work)^(1/.N),
  other = prod(other)^(1/.N),
  school = prod(school)^(1/.N) # school is often 0, so weight slightly differently
), by=.(region=abbr) ]
...
us <- function(sdt.row) {
  rep(sdt.row[, as.numeric(.SD), .SDcols = grep("^u_",names(sdt.row))], each = 2)
}
...
assumed.AR <- AR.dt[timing, on=.(province=abbr, date=start)][,.(province, date, agegrp, escapable, non_reinfectable, inc.room = 1-non_reinfectable-escapable)]
